...
func (self *{{../namePascalCase}}) on{{trigger}}() (err error){
	...
	{{aggregate.nameCamelCase}} := &external.{{aggregate.namePascalCase}}{}
	resp := external.Get{{aggregate.namePascalCase}}({{aggregate.nameCamelCase}}.{{aggregate.keyFieldDescriptor.namePascalCase}})
	fmt.Println(resp)
...
func (self *{{../namePascalCase}}) {{namePascalCase}}({{#if (has fieldDescriptors)}}{{nameCamelCase}}Command *{namePascalCase}}Command{{/if}}){
	...
	Publish({{eventValue.nameCamelCase}})
	{{#relationCommandInfo}}
	{{#commandValue}}
...
{{#policyList}}
{{#relationEventInfo}}
func (self *{{../namePascalCase}}) {{../namePascalCase}}({{eventValue.nameCamelCase}} *{{eventValue.namePascalCase}}){
	/** Example 1:  new item
